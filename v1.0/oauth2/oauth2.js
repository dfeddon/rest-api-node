...
server.exchange(oauth2orize.exchange.password(function(client, username, password, scope, done) {//console.log(":: "+client+"/"+username+"/"+password)
    UserModel.findOne({ username: username }, function(err, user) {
        if (err) { return done(err); }
        if (!user) { return done(null, false); }
        //console.log("***************** "+user.username+"/"+username+"/"+password)
